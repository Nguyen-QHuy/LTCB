...
void demsolan()
{
    int So_lan = 0;
    FILE *file;
    file = fopen("solan.txt", "r"); // mo file len de doc
    if (file != NULL)
    {
        fscanf(file, "%d", &So_lan); // doc va gan gia tri tu file vao bien So_lan
    ...
    if (file != NULL)
    {
        fprintf(file, "%d", So_lan); // ghi gia tri vao file
        fclose(file);
    }
}
...
void Tao_min()
{
    ...
    while (i < So_min) // khi so min chua dat toi da thi tiep tuc them
    {
        x = rand() % col;
        y = rand() % col;
        if (MineMap[x][y] == EMPTY) // neu vi tri x;y chua co min thi them min
        {
            MineMap[x][y] = MINE;
            i++;
        }
